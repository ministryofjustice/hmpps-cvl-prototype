...
//Change the team view on create case list for PP v20+
router.post(version + '/change-team', function(req, res) {
  res.redirect(version + '/list-team-view');
});


//Change the team view on vary case list for PP v20+
router.post(version + '/vary/change-team', function(req, res) {
  res.redirect(version + '/vary/list-team-view');
});
...
//router.post('/VLO-consulting', function (req, res) {
  //var VLOConsulting = req.session.data['vloconsulting']

  // Check whether the variable matches a condition
  //if (VLOConsulting == "yes"){
   
   // Send user to next page
    //res.redirect('v20/meet')
  //} else {
...
//router.post('/VLO-consulting-reason', function (req, res) {
  //var VLOConsultingReason = req.session.data['reason-textbox']

  // Check whether the variable matches a condition
  //if (VLOConsultingReason == "yes"){
   
   // Send user to next page
    //res.redirect('v20/check-your-answers')
  //} else {
    // Send user to ineligible page
    //res.redirect('v20/check-your-answers')
  //}
...
//Exclusion zone
//Add another exclusionzone

router.post('/add-exclusionzone', function (req, res) {
  var exclusionzoneAddAnother = req.session.data['addanotherexclusionzone']

  // Check whether the variable matches a condition
  if (exclusionzoneAddAnother == "yes"){
   
   // Send user to next page
    res.redirect('v20/additional/8a-map2')
  } else {
    // Send user to ineligible page
    res.redirect('v20/check-your-answers')
  }
...
router.post('/delete-exclusionzone', function (req, res) {

  // Make a variable and give it the value from 'how-many-letters'
  var exclusionzoneNotNeeded = req.session.data['exclusionzone-not-needed']

  // Check whether the variable matches a condition
  if (exclusionzoneNotNeeded == "yes"){
    // Send user to next page
    res.redirect('v20/additional/8a-overview')
  } else {
    // Send user to ineligible page
    res.redirect('v20/additional/8a-overview')
  }
...
// Exclusion zone VLO agreement question

router.post('/vlo-agreement-question', function (req, res) {

  let agreementAnswer = req.session.data['vloagreement'];

  if (agreementAnswer == "yes") {
    res.redirect('v20/check-your-answers')
  } else {
    res.redirect('v20/additional/8a-VLO-outcome')
  } 
});

// Run this code when a form is submitted to 'parole-outcome-letter-answer'
router.post('/delete-exclusionzone-maptwo', function (req, res) {

  // Make a variable and give it the value from 'how-many-letters'
  var exclusionzoneNotNeededMaptwo = req.session.data['exclusionzone-not-needed-maptwo']

  // Check whether the variable matches a condition
  if (exclusionzoneNotNeededMaptwo == "yes"){
    // Send user to next page
    res.redirect('v20/additional/8a-overview')
  } else {
    // Send user to ineligible page
...
router.post(version + '/question-activate', function(req, res) {
 ...
 //NEW PAGE FOR CREATE JOURNEY
router.post(version + '/question-checklist', function(req, res) {
  //If other is selected route to other page_title
  //otherwise continue to CYA
  var checkOTHER = req.session.data['checkOTHER'];

  if (checkOTHER=="true"){
    res.redirect(version + '/checklist-other');
  }
  else{
    res.redirect(version + '/meet');
  }
